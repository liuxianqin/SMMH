...
class sjf:
	def __init__(self,jcb_list,n):
		...
		self.do_list=[]
	def slicing(self):
		self.tlist=self.jlist
		self.tlist=sorted(self.tlist,key=lambda jcb: jcb.submit_time)
		self.time_line=self.tlist[0].submit_time+self.tlist[0].need_time
		for i in tlist:
			pass
	...
	def working(self):
		...
		for i in range(len(self.jlist)):
			...
			#self.time_flag=self.jlist[0].submit_time+self.jlist[0].need_time
